...
module.exports.users_get = async (req, res, next) => {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    const shipments = await Entry.countDocuments({
      "details.Customer Email": users[i].email,
    });
...
module.exports.upload_post = async (req, res, next) => {
  try {
    ...
    if (req.files) {
      ...
      if (
        !(
          fileName.toLowerCase().startsWith("ff") ||
          fileName.toLowerCase().startsWith("cc")
        )
      )
        errors.push("Not an FF or CC file, File name must start with FF or CC");
    } else {
    ...
    if (errors.length > 0) {
      ...
      if (sheets.length !== 4)
        errors.push("The number of sheets are less than or more than 4");
      if (!sheets.some((s) => s == "Air Export"))
        errors.push("Cannot find Air Export Sheet");
      if (!sheets.some((s) => s == "Air Import"))
        errors.push("Cannot find Air Import Sheet");
      if (!sheets.some((s) => s == "Sea Export"))
        errors.push("Cannot find Sea Export Sheet");
      if (!sheets.some((s) => s == "Sea Import"))
        errors.push("Cannot find Sea Import Sheet");
      ...
      if (errors.length > 0) {
        ...
        for (const sheet in results) {
          ...
          for (let i = 1; i < results[sheet].length; i++) {
            ...
            for (const detailKey in details) {
              entry.details[details[detailKey].replace(/\./gi, "")] =
                results[sheet][i][detailKey];
            }
        ...
        req.flash(
          "success_msg",
          `Added ${result.created}, Updated ${result.updated} Entries`
        );
        res.redirect("/admin/data");
      }
    }
